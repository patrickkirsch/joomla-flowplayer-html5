...
class plgSystemFlowplayerReloaded extends JPlugin {
	...
	function onAfterRoute() {
		...
		if (stripos($juri->getPath(), '/administrator/') !== false)
			return;
		...
		if ($this->params->get('ipad'))
			$document->addScript(_URL_FPP_ . 'swfplugins/flowplayer.ipad-3.2.2.min.js');
	}
	...
	protected function buildHtml4Instance(&$matches) {
		...
		if (strtolower(substr($filename, -4)) != ".rss")
			$href = 'href="' . $filename . '"';
		else
			$href = '';
		$id = crc32($filename);

		if (!$p['img'] && $this->params->get('globalimg', ''))
			$p['img'] = $this->params->get('globalimg', '');
		...
		if (!$popup) {
			...
			$video = '<' . $elem . ' class="flowplayer' . $id . '" id="flowplayer' . $id . '" style="width: '
			...
			if ($dlink && $href)
				$video .= '<br /><a ' . $href . ' class="flowplayer_dlink" >' . $this->params->get('dlinktext', "Download") . '</a>';
		}
		else {
			$p['autoplay'] = 1;
			...
			//build alternative player
			$alternative_player = "";
			if (!empty($alternative_mp4) || !empty($alternative_webm) || !empty($alternative_ogg)) {
				$alternative_player = <<<EOD
					<div id="video_container_$id" style="display:none; width:100%; height:100%">
 <video id="video_$id" style="display:none;" width="100%" height="100%" controls  class="player">
EOD;

				$alternative_player .=!empty($alternative_mp4) ? '<source src="' . $alternative_mp4 . '" type="video/mp4" />' : "";
				$alternative_player .=!empty($alternative_webm) ? '<source src="' . $alternative_webm . '" type="video/webm" />' : "";
				$alternative_player .=!empty($alternative_ogg) ? '<source src="' . $alternative_ogg . '" type="video/ogg" />' : "";
				$alternative_player .= "</video></div>";
			}

			//build video block
			$video = '<' . $elem . ' class="flowplayer' . $id . '" id="flowplayer' . $id . '" style="width: '
					. $pwidth . 'px; height: ' . $pheight . 'px;	display: block;">' . $alternative_player . '</' . $elem . '>';
			if (!($this->params->get('noscript') == 1))
				$video .= $this->buildScript($p, $id, "backgroundImage: '" . $p['img'] . "'", $filename);

			//build wrapper
			$video = '<a id="fprrpopup_' . $id . '" class="fprrpopup' . $id . '" href="#fprrpopup' . $id . '" style="display: block; width: ' . $width . 'px;
		...
		if ($center)
			$video = '<div id="container_' . $id . '" style="display: block; width: 100%;">' . $video . '</div><style>.flowplayer' . $id . ',.fprrpopup' . $id . '{margin: 0px auto;}</style>';
		//var_dump($p);
		return $video;
	}
	...
	protected function buildScript($p, $id = '', $img = '', $filename = '') {
		...
		if ($filename) {
			...
			if (strpos(strtolower($filename), "rtmp") === FALSE) {
				$fpp_clip = "$fpp_clip,
								  url: '$filename'";
			}
		}
		...
		if ($this->params->get('ipad'))
			$fpp_jssuffix = ".each(function(){this.ipad(" .
		...
		if ($fpp_viral && ( $fpp_plugins || $this->params->get('subtitles') || $p['subtitles']))
			$fpp_viral .= ",\n";
		...
		// RTMP handling
		if (strpos(strtolower($filename), "rtmp") == 0) {
			// e.g.
			$aUrl = parse_url($filename); // hostname
			$aPath = explode('/', $aUrl['path']); // index 0 is connector
			$plugin_rtmp = "rtmp: { " .
					"url: '" . _URL_FPP_ . "swfplugins/flowplayer.rtmp-3.2.13.swf', " .
					"netConnectionUrl: 'rtmp://" . $aUrl['host'] . "/" . $aPath[1] . "' }";
			$fpp_clip .= ",\n provider: 'rtmp' ";
			$aPathinfo = pathinfo($aUrl['path']);
			// scheme is mp4:my_clip.mp4
			$playlist = sprintf(" playlist: [ '%s' ] \n, ", $aPathinfo['filename'] . "." . $aPathinfo['extension']);
		}


		//Build javascript finally
		$script = '<script type="text/javascript">
	$("fprrpopup_' . $id . '").addEvent("click", function (event) {
		if (typeof Browser.Plugins.Flash !== "undefined" && Browser.Plugins.Flash.version > 0) {
			$f("' . $elem . '.flowplayer' . $id . '", {src: "' . _URL_FPP_ . 'players/' . $this->params->get('swf') . '", wmode: "' . $fpp_wmode . '"}, {
				...
				plugins: {
					' . $fpp_viral . $fpp_plugins . $plugin_rtmp . '
				}' . $fpp_config . '
			})' . $fpp_jssuffix . ';
		}
		else
